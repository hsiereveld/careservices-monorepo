...
// Test connection function with better error reporting and retry logic
const testConnection = async (retries = 3) => {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (fetchError) {
        console.error(`Supabase connection test failed (attempt ${attempt}):`, fetchError);
        if (attempt === retries) {
...
// Initialize connection test on module load with retry logic
