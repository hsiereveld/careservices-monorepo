...
import { User, Mail, Phone, Calendar, FileText, Lock, Save, AlertCircle, CheckCircle, Camera, ToggleLeft, ToggleRight, Euro, Briefcase, FileCheck, CreditCard, Building, Trash2, Loader2, Upload, MapPin } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { supabase, Profile, ServiceProvider } from '../lib/supabase';
import { useAdmin } from '../hooks/useAdmin';
import { v4 as uuidv4 } from 'uuid';
import { validateProviderProfile } from '../utils/profileValidation';
...
export function ProfilePage() {
  ...
  // Update validation state when businessData changes
  useEffect(() => {
    if (provider) {
      // Create a merged object with current provider data and form data
      const mergedProvider = {
        ...provider,
        ...businessData
      };
      
      const validation = validateProviderProfile(mergedProvider);
    ...
    try {
      ...
      if (data) {
        ...
        setBusinessData({
          ...
          city: data.city || 'Pinoso',
          postal_code: data.postal_code || '',
          email: data.email || '',
          phone: data.phone || ''
        });
        
        // Run validation on initial data
        const validation = validateProviderProfile(data);
        setValidationState(validation);
      }
    } catch (err: any) {
            ...
            {activeTab === 'business' && isBusinessTabVisible && (
              <form onSubmit={handleBusinessSubmit} className="space-y-6">
                ...
                <div className="bg-blue-50 rounded-2xl p-6 mb-6">
                  <h3 className="text-lg font-semibold text-blue-800 mb-2">Profiel Compleetheid</h3>
                  <div className="w-full bg-gray-200 rounded-full h-4 mb-4">
                    <div 
                      className={`h-4 rounded-full ${
                        validationState.completionPercentage >= 70 ? 'bg-green-500' : 
                        validationState.completionPercentage >= 40 ? 'bg-yellow-500' : 
                        'bg-red-500'
                      }`}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm text-blue-700">Profiel compleetheid</span>
                    <span className={`text-sm font-medium ${
                      validationState.completionPercentage >= 70 ? 'text-green-600' : 
                      validationState.completionPercentage >= 40 ? 'text-yellow-600' : 
                      'text-red-600'
                    }`}>
                      {validationState.completionPercentage}% ({validationState.validCount}/{validationState.requiredCount} verplichte velden)
                    </span>
                  </div>
                  <p className="text-blue-700 text-sm">
                    {validationState.isProfileComplete 
                      ? 'Je profiel is voldoende ingevuld om zichtbaar te zijn voor klanten.' 
                      : 'Vul de verplichte velden in om je profiel zichtbaar te maken voor klanten.'}
                  </p>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-12 pr-4 py-4 bg-primary-50 border ${
                          validationState.validationResults?.business_name?.isValid === false 
                            ? 'border-red-300 focus:ring-red-500' 
                            : 'border-primary-200 focus:ring-primary-500'
                        } rounded-2xl text-text-primary placeholder-text-light focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300`}
                        placeholder="Bedrijfsnaam of handelsnaam"
                      />
                    </div>
                    {validationState.validationResults?.business_name?.isValid === false && (
                      <p className="mt-1 text-sm text-red-600">{validationState.validationResults.business_name.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-12 pr-4 py-4 bg-primary-50 border ${
                          validationState.validationResults?.hourly_rate?.isValid === false 
                            ? 'border-red-300 focus:ring-red-500' 
                            : 'border-primary-200 focus:ring-primary-500'
                        } rounded-2xl text-text-primary placeholder-text-light focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300`}
                        placeholder="0.00"
                    ...
                    {validationState.validationResults?.hourly_rate?.isValid === false && (
                      <p className="mt-1 text-sm text-red-600">{validationState.validationResults.hourly_rate.message}</p>
                    )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      ...
                      className={`w-full pl-12 pr-4 py-4 bg-primary-50 border ${
                        validationState.validationResults?.description?.isValid === false 
                          ? 'border-red-300 focus:ring-red-500' 
                          : 'border-primary-200 focus:ring-primary-500'
                      } rounded-2xl text-text-primary placeholder-text-light focus:outline-none focus:ring-2 focus:border-transparent h-32 resize-none transition-all duration-300`}
                      placeholder="Beschrijf je bedrijf of dienstverlening..."
                    />
                  </div>
                  {validationState.validationResults?.description?.isValid === false && (
                    <p className="mt-1 text-sm text-red-600">{validationState.validationResults.description.message}</p>
                  )}
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-12 pr-4 py-4 bg-primary-50 border ${
                          validationState.validationResults?.email?.isValid === false 
                            ? 'border-red-300 focus:ring-red-500' 
                            : 'border-primary-200 focus:ring-primary-500'
                        } rounded-2xl text-text-primary placeholder-text-light focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300`}
                        placeholder="zakelijk@email.com"
                      />
                    </div>
                    {validationState.validationResults?.email?.isValid === false && (
                      <p className="mt-1 text-sm text-red-600">{validationState.validationResults.email.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-12 pr-4 py-4 bg-primary-50 border ${
                          validationState.validationResults?.phone?.isValid === false 
                            ? 'border-red-300 focus:ring-red-500' 
                            : 'border-primary-200 focus:ring-primary-500'
                        } rounded-2xl text-text-primary placeholder-text-light focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300`}
                    ...
                    {validationState.validationResults?.phone?.isValid === false && (
                      <p className="mt-1 text-sm text-red-600">{validationState.validationResults.phone.message}</p>
                    )}
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-12 pr-4 py-4 bg-primary-50 border ${
                          validationState.validationResults?.city?.isValid === false 
                            ? 'border-red-300 focus:ring-red-500' 
                            : 'border-primary-200 focus:ring-primary-500'
                        } rounded-2xl text-text-primary placeholder-text-light focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300`}
                        placeholder="Pinoso"
                    ...
                    {validationState.validationResults?.city?.isValid === false && (
                      <p className="mt-1 text-sm text-red-600">{validationState.validationResults.city.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-12 pr-4 py-4 bg-primary-50 border ${
                          validationState.validationResults?.bank_account_number?.isValid === false 
                            ? 'border-red-300 focus:ring-red-500' 
                            : 'border-primary-200 focus:ring-primary-500'
                        } rounded-2xl text-text-primary placeholder-text-light focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300`}
                    ...
                    {validationState.validationResults?.bank_account_number?.isValid === false && (
                      <p className="mt-1 text-sm text-red-600">{validationState.validationResults.bank_account_number.message}</p>
                    )}
                  </div>
