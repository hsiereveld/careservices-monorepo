/*
  ...
  1. New Tables
    ...
    - `bookings` - Main bookings table with all booking details
    - `booking_status_history` - Track status changes
    - `booking_reviews` - Customer reviews for completed bookings

  2. Security
    - Enable RLS on all tables
    - Add comprehensive policies for customers, providers, and admins
    - Proper access control for all user types

  3. Features
    - Booking status tracking with history
...
CREATE POLICY "Professionals can manage provider profiles"
  ...
  FOR ALL
  TO authenticated
  USING (is_professional() AND user_id = auth.uid())
  WITH CHECK (is_professional() AND user_id = auth.uid());
...
CREATE POLICY "Clients can manage own bookings"
  ...
  FOR ALL
  TO authenticated
  USING (is_client() AND customer_id = auth.uid())
  WITH CHECK (is_client() AND customer_id = auth.uid());
...
CREATE POLICY "Professionals can manage assigned bookings"
  ...
  FOR ALL
  TO authenticated
  USING (is_professional() AND provider_id IN (
    SELECT id FROM service_providers WHERE user_id = auth.uid()
  ))
  WITH CHECK (is_professional() AND provider_id IN (
    SELECT id FROM service_providers WHERE user_id = auth.uid()
  ));
