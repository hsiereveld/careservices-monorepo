/*
# Fix Admin Views Security Settings

1. Security Improvements
   - Changed admin views to use SECURITY INVOKER instead of SECURITY DEFINER
   - Maintained existing functionality while improving security
   - Added explicit security checks in view definitions

2. Changes
   - Modified admin_users_overview view
   - Modified admin_tasks_overview view
   - Modified users_with_roles view
   - Updated get_admin_users_overview function

3. Security
   - Views now respect the permissions of the querying user
   - Explicit WHERE clauses still filter data based on user role
*/

-- Create a function to check if the current user has admin or backoffice privileges
...
-- Recreate admin_users_overview with SECURITY INVOKER
...
WHERE 
  has_admin_access() -- Only show data if user has admin privileges
GROUP BY 
...
-- Recreate users_with_roles with SECURITY INVOKER
...
-- Recreate admin_tasks_overview with SECURITY INVOKER
...
GRANT EXECUTE ON FUNCTION public.has_admin_access TO authenticated, anon;
