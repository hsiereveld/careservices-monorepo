/*
  # Add custom fields to provider_services table
  
  1. New Fields
    - Added custom fields to provider_services table:
      - `custom_name` (text)
      - `custom_short_description` (text)
      - `custom_full_description` (text)
      - `custom_price_unit` (price_unit_type)
      - `custom_duration_minutes` (integer)
      - `custom_target_audience` (text)
      - `custom_image_url` (text)
  
  2. Indexes
    - Added index on `custom_name` for faster queries
  
  3. Security
    - Added policy for professionals to manage their own services
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE polname = 'Professionals can manage own services'
    AND polrelid = (SELECT oid FROM pg_class WHERE relname = 'provider_services' AND relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public'))
  ) THEN
